...
public class CoverFilterFluid extends AbstractCoverAttachment {
	...
	public boolean interceptFluidFill(byte aCoverSide, CoverData aData, byte aSide, FluidStack aFluidToFill) {
		...
		if (aData.mNBTs[aCoverSide] == null || !aData.mNBTs[aCoverSide].hasKey("gt6.filter.fluid")) return aData.mVisuals[aCoverSide] == 0;
		return (aData.mVisuals[aCoverSide] == 0) != FL.equal(FL.load(aData.mNBTs[aCoverSide], "gt6.filter.fluid"), aFluidToFill, T);
	}
	@Override
	public boolean interceptFluidDrain(byte aCoverSide, CoverData aData, byte aSide, FluidStack aFluidToDrain) {
		...
		if (aData.mNBTs[aCoverSide] == null || !aData.mNBTs[aCoverSide].hasKey("gt6.filter.fluid")) return aData.mVisuals[aCoverSide] == 0;
		return (aData.mVisuals[aCoverSide] == 0) != FL.equal(FL.load(aData.mNBTs[aCoverSide], "gt6.filter.fluid"), aFluidToDrain, T);
	}
