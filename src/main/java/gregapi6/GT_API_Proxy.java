...
import gregapi6.block.multitileentity.MultiTileEntityItemInternal;
...
import gregapi6.item.IItemRottable.RottingUtil;
...
import gregapi6.oredict.listeners.IOreDictListenerItem;
import gregapi6.player.EntityFoodTracker;
import gregapi6.random.IHasWorldAndCoords;
import gregapi6.tileentity.ITileEntityErrorable;
import gregapi6.tileentity.ITileEntityGUI;
import gregapi6.tileentity.ITileEntityNeedsSaving;
...
import gregapi6.tileentity.inventories.ITileEntityBookShelf;
...
public abstract class GT_API_Proxy extends Abstract_Proxy implements IGuiHandler, IFuelHandler, IWorldGenerator {
	...
	public void onProxyBeforeServerStarted(Abstract_Mod aMod, FMLServerStartedEvent aEvent) {
		...
		if (mSaveLocation == null) {
			...
			new File(mSaveLocation, "gregtech6").mkdirs();
					...
					for (ItemStack tOutput : tStacks) {
						if (OreDictManager.INSTANCE.isOreDictItem(tOutput)) {
							...
							if (CS.CODE_CLIENT) {
								...
								UT.NBT.setBoolean(UT.NBT.getNBT(tOutput), "gt6.err.oredict.output", T);
							}
						} else {
							OM.set(tOutput);
						}
