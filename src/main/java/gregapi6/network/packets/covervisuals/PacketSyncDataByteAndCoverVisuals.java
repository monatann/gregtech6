 ...
 */
public class PacketSyncDataByteAndCoverVisuals extends PacketSyncDataByte {
	public short mCoverVisuals[];
	public boolean mVisualsToSync[];
	
	public PacketSyncDataByteAndCoverVisuals(int aDecoderType) {
		super(aDecoderType);
	}
	
	public PacketSyncDataByteAndCoverVisuals(int aX, int aY, int aZ, byte aData, CoverData aCoverData) {
	...
	private PacketSyncDataByteAndCoverVisuals(int aX, int aY, int aZ, byte aData, ByteArrayDataInput aCovers) {
		super(aX, aY, aZ, aData);
	...
	public PacketCoordinates decode2(int aX, int aY, int aZ, ByteArrayDataInput aData) {
		return new PacketSyncDataByteAndCoverVisuals(aX, aY, aZ, aData.readByte(), aData);
	}
	...
	public void process(IBlockAccess aWorld, INetworkHandler aNetworkHandler) {
		if (aWorld != null) {
			...
			if (tBlock instanceof IBlockSyncDataAndCoversAndIDs) ((IBlockSyncDataAndCoversAndIDs)tBlock).receiveDataByte(aWorld, mX, mY, mZ, mData, aNetworkHandler, mCoverVisuals, mVisualsToSync);
		}
	}
}
