...
package gregapi6.network.packets.covervisuals;
...
public class PacketSyncDataIntegerAndCoverVisuals extends PacketSyncDataInteger {
	...
	public PacketSyncDataIntegerAndCoverVisuals(int aDecoderType) {
		super(aDecoderType);
	}
	
	public PacketSyncDataIntegerAndCoverVisuals(int aX, int aY, int aZ, int aData, CoverData aCoverData) {
		super(aX, aY, aZ, aData);
		mVisualsToSync = aCoverData.mVisualsToSync;
		mCoverVisuals = aCoverData.mVisuals;
	}
	public PacketSyncDataIntegerAndCoverVisuals(ChunkCoordinates aCoords, int aData, CoverData aCoverData) {
		super(aCoords, aData);
		mVisualsToSync = aCoverData.mVisualsToSync;
		mCoverVisuals = aCoverData.mVisuals;
	}
	
	private PacketSyncDataIntegerAndCoverVisuals(int aX, int aY, int aZ, int aData, ByteArrayDataInput aCovers) {
		super(aX, aY, aZ, aData);
	...
	public PacketCoordinates decode2(int aX, int aY, int aZ, ByteArrayDataInput aData) {
		return new PacketSyncDataIntegerAndCoverVisuals(aX, aY, aZ, aData.readInt(), aData);
	}
	...
	public void process(IBlockAccess aWorld, INetworkHandler aNetworkHandler) {
		if (aWorld != null) {
			...
			if (tBlock instanceof IBlockSyncDataAndCoversAndIDs) ((IBlockSyncDataAndCoversAndIDs)tBlock).receiveDataInteger(aWorld, mX, mY, mZ, mData, aNetworkHandler, mCoverVisuals, mVisualsToSync);
		}
	}
}
