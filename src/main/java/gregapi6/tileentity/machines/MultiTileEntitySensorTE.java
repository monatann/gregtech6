...
import gregapi6.computer.ITileEntityComputerizable;
...
public abstract class MultiTileEntitySensorTE extends MultiTileEntitySensor implements ITileEntityComputerizable, ITileEntityServerTickPost {
	...
	public void readFromNBT2(NBTTagCompound aNBT) {
		...
		mCurrentMax = aNBT.getInteger("gt6.sensor.max");
		mCurrentValue = aNBT.getInteger("gt6.sensor.value");
		mIndex = aNBT.getInteger("gt6.sensor.index");
		mValues = aNBT.getIntArray("gt6.sensor.array");
	...
	public void writeToNBT2(NBTTagCompound aNBT) {
		...
		UT.NBT.setNumber(aNBT, "gt6.sensor.max", mCurrentMax);
		UT.NBT.setNumber(aNBT, "gt6.sensor.value", mCurrentValue);
		UT.NBT.setNumber(aNBT, "gt6.sensor.index", mIndex);
		aNBT.setIntArray("gt6.sensor.array", mValues);
	}
	
	@Override
	public NBTTagCompound writeItemNBT2(NBTTagCompound aNBT) {
		aNBT = super.writeItemNBT2(aNBT);
		if (mIndex != 0) aNBT.setInteger("gt6.sensor.index", mIndex);
		if (mValues.length > 1) aNBT.setIntArray("gt6.sensor.array", new int[mValues.length]);
		return aNBT;
	}
