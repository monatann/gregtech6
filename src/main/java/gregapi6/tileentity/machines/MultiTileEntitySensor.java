...
public abstract class MultiTileEntitySensor extends TileEntityBase10FacingDouble implements ITileEntityQuickObstructionCheck, IMTE_SetBlockBoundsBasedOnState, IMTE_GetCollisionBoundingBoxFromPool, IMTE_GetSelectedBoundingBoxFromPool, IMTE_SyncDataShort {
	...
	static {LH.add("gt6.tooltip.sensor.screwdrive.buttons"   , "Use Screwdriver on Buttons to enable Averaging Mode.");}
	static {LH.add("gt6.tooltip.sensor.screwdrive.display"   , "Use Screwdriver on Display toggle Hexadecimal Display.");}
	static {LH.add("gt6.tooltip.sensor.screwdrive.modes"     , "Use Screwdriver on anything else to switch Modes.");}
	
	@Override
	public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
		aList.add(Chat.CYAN     + getSensorDescription());
		aList.add(Chat.ORANGE   + LH.get(LH.NO_GUI_CLICK_TO_INTERACT));
		aList.add(Chat.DGRAY    + LH.get("gt6.tooltip.sensor.screwdrive.buttons"));
		aList.add(Chat.DGRAY    + LH.get("gt6.tooltip.sensor.screwdrive.display"));
		aList.add(Chat.DGRAY    + LH.get("gt6.tooltip.sensor.screwdrive.modes"));
	...
	public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
		...
		if (aShouldSideBeRendered[aSide] && aRenderPass == 0) {
			if (aSide == OPPOSITES[mFacing]) return BlockTextureMulti.get(BlockTextureDefault.get(getTextureBack(), mRGBa), BlockTextureDefault.get(getOverlayBack()));
	...
	@Override public byte isProvidingWeakPower2(byte aSide) {return aSide == OPPOSITES[mSecondFacing] ? 0 : mRedstone;}
