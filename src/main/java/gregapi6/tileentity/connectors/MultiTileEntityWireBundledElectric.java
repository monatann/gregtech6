...
public class MultiTileEntityWireBundledElectric extends MultiTileEntityWireElectric {
	...
	public void onTick2(long aTimer, boolean aIsServerSide) {
		...
		if (aIsServerSide) {
			...
			if (EnergyCompat.IC_ENERGY) for (byte tSide : ALL_SIDES_VALID) if (canAcceptEnergyFrom(tSide)) {
				...
				if (!(tDelegator.mTileEntity instanceof gregapi6.tileentity.ITileEntityEnergy)) {
	...
	@Override public String getTileEntityName               () {return "gt6.multitileentity.connector.wire.bundled.electric";}
}
