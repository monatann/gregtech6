...
import gregapi6.block.prefixblock.PrefixBlockFallingEntity;
...
import gregapi6.compat.ICompat;
...
import gregapi6.cover.ICover;
import gregapi6.cover.covers.CoverRedstoneRepeater;
import gregapi6.cover.covers.CoverRedstoneTorch;
import gregapi6.data.AM;
...
import gregapi6.enchants.Enchantment_EnderDamage;
import gregapi6.enchants.Enchantment_Radioactivity;
import gregapi6.enchants.Enchantment_SlimeDamage;
import gregapi6.enchants.Enchantment_WerewolfDamage;
import gregapi6.item.ItemEmptySlot;
import gregapi6.item.ItemFluidDisplay;
import gregapi6.item.ItemIntegratedCircuit;
import gregapi6.lang.LanguageHandler;
import gregapi6.load.LoaderOreDictReRegistrations;
import gregapi6.log.LogBuffer;
import gregapi6.log.LoggerPlayerActivity;
import gregapi6.network.NetworkHandler;
import gregapi6.network.packets.PacketBlockError;
import gregapi6.network.packets.PacketBlockEvent;
import gregapi6.network.packets.PacketConfig;
import gregapi6.network.packets.PacketDeathPoint;
import gregapi6.network.packets.PacketItemStackChat;
import gregapi6.network.packets.PacketPrefix;
import gregapi6.network.packets.PacketSound;
import gregapi6.network.packets.covers.PacketSyncDataByteAndIDsAndCovers;
import gregapi6.network.packets.covers.PacketSyncDataByteArrayAndIDsAndCovers;
import gregapi6.network.packets.covers.PacketSyncDataIDsAndCovers;
import gregapi6.network.packets.covers.PacketSyncDataIntegerAndIDsAndCovers;
import gregapi6.network.packets.covers.PacketSyncDataLongAndIDsAndCovers;
import gregapi6.network.packets.covers.PacketSyncDataShortAndIDsAndCovers;
import gregapi6.network.packets.covervisuals.PacketSyncDataByteAndCoverVisuals;
import gregapi6.network.packets.covervisuals.PacketSyncDataByteArrayAndCoverVisuals;
import gregapi6.network.packets.covervisuals.PacketSyncDataCoverVisuals;
import gregapi6.network.packets.covervisuals.PacketSyncDataIntegerAndCoverVisuals;
import gregapi6.network.packets.covervisuals.PacketSyncDataLongAndCoverVisuals;
import gregapi6.network.packets.covervisuals.PacketSyncDataShortAndCoverVisuals;
import gregapi6.network.packets.data.PacketSyncDataByte;
import gregapi6.network.packets.data.PacketSyncDataByteArray;
import gregapi6.network.packets.data.PacketSyncDataInteger;
import gregapi6.network.packets.data.PacketSyncDataLong;
import gregapi6.network.packets.data.PacketSyncDataName;
import gregapi6.network.packets.data.PacketSyncDataShort;
import gregapi6.network.packets.ids.PacketSyncDataByteAndIDs;
import gregapi6.network.packets.ids.PacketSyncDataByteArrayAndIDs;
import gregapi6.network.packets.ids.PacketSyncDataIDs;
...
import gregapi6.network.packets.ids.PacketSyncDataShortAndIDs;
...
import gregapi6.recipes.AdvancedCraftingShaped;
import gregapi6.recipes.AdvancedCraftingShapeless;
import gregapi6.recipes.AdvancedCraftingTool;
import gregapi6.recipes.AdvancedCraftingXToY;
import gregapi6.render.IRenderedBlockObject.ErrorRenderer;
...
import gregapi6.worldgen.GT6WorldGenerator;
...
public class GT_API extends Abstract_Mod {
	...
	public void onPreLoad(FMLPreInitializationEvent aEvent) {
		...
		DirectoriesGT.CONFIG_GT = new File(DirectoriesGT.CONFIG, "GregTech6");
		if (!DirectoriesGT.CONFIG_GT.exists()) DirectoriesGT.CONFIG_GT = new File(DirectoriesGT.CONFIG, "gregtech6");
...
//  }
	...
	public void onModPreInit2(FMLPreInitializationEvent aEvent) {
		...
		tFile = new File(DirectoriesGT.CONFIG_GT, "gregtech6.cfg");
		...
		tFile = new File(DirectoriesGT.LOGS, "gregtech6.log");
		...
		ConfigsGT.CLIENT = new Config(DirectoriesGT.MINECRAFT, "gregtech6.cfg");
		...
		if (tMainConfig.get("compat", "IC2Classic"          , T).getBoolean(T)) ICompat.COMPAT_CLASSES.add((ICompat)UT.Reflection.callConstructor("gregapi6.compat.industrialcraft.CompatIC2C", 0, null, D2));
		if (tMainConfig.get("compat", "IC2EnergyItems"      , T).getBoolean(T)) ICompat.COMPAT_CLASSES.add(COMPAT_EU_ITEM   = (ICompatIC2EUItem )UT.Reflection.callConstructor("gregapi6.compat.industrialcraft.CompatIC2EUItem" , 0, null, D2));
		if (tMainConfig.get("compat", "IndustrialCraft2"    , T).getBoolean(T)) ICompat.COMPAT_CLASSES.add(COMPAT_IC2       = (ICompatIC2       )UT.Reflection.callConstructor("gregapi6.compat.industrialcraft.CompatIC2"       , 0, null, D2));
		if (tMainConfig.get("compat", "ThaumCraft"          , T).getBoolean(T)) ICompat.COMPAT_CLASSES.add(COMPAT_TC        = (ICompatTC        )UT.Reflection.callConstructor("gregapi6.compat.thaumcraft.CompatTC"             , 0, null, D2));
		if (tMainConfig.get("compat", "BuildCraft"          , T).getBoolean(T)) ICompat.COMPAT_CLASSES.add(COMPAT_BC        = (ICompatBC        )UT.Reflection.callConstructor("gregapi6.compat.buildcraft.CompatBC"             , 0, null, D2));
		if (tMainConfig.get("compat", "ComputerCraft"       , T).getBoolean(T)) ICompat.COMPAT_CLASSES.add(COMPAT_CC        = (ICompatCC        )UT.Reflection.callConstructor("gregapi6.compat.computercraft.CompatCC"          , 0, null, D2));
		if (tMainConfig.get("compat", "Forestry"            , T).getBoolean(T)) ICompat.COMPAT_CLASSES.add(COMPAT_FR        = (ICompatFR        )UT.Reflection.callConstructor("gregapi6.compat.forestry.CompatFR"               , 0, null, D2));
		if (tMainConfig.get("compat", "GalactiCraft"        , T).getBoolean(T)) ICompat.COMPAT_CLASSES.add(COMPAT_GC        = (ICompatGC        )UT.Reflection.callConstructor("gregapi6.compat.galacticraft.CompatGC"           , 0, null, D2));
		...
		ConfigsGT.OVERPOWERED = ConfigsGT.MACHINES = ConfigsGT.SPECIAL = new Config("gregtech6.cfg");
		...
		RecipeSorter.register("gregtech6:shaped"         , AdvancedCraftingShaped.class              , net.minecraftforge.oredict.RecipeSorter.Category.SHAPED       , "after:minecraft:shaped before:minecraft:shapeless");
		RecipeSorter.register("gregtech6:shapeless"      , AdvancedCraftingShapeless.class           , net.minecraftforge.oredict.RecipeSorter.Category.SHAPELESS    , "after:minecraft:shapeless");
		RecipeSorter.register("gregtech6:1ToY"           , AdvancedCrafting1ToY.class                , net.minecraftforge.oredict.RecipeSorter.Category.SHAPELESS    , "after:gregtech6:shapeless");
		RecipeSorter.register("gregtech6:XToY"           , AdvancedCraftingXToY.class                , net.minecraftforge.oredict.RecipeSorter.Category.SHAPELESS    , "after:gregtech6:1ToY");
		RecipeSorter.register("gregtech6:tool"           , AdvancedCraftingTool.class                , net.minecraftforge.oredict.RecipeSorter.Category.SHAPELESS    , "after:gregtech6:1ToY");
		...
		GameRegistry.registerTileEntity(PrefixBlockTileEntity.class, "gt6.MetaBlockTileEntity");
		// Creating and loading the Lang File.
		if (CODE_CLIENT) {
			for(int i=0;i<Locale.getAvailableLocales().length;i++) {
				tFile = new File("resources/gregtech6/lang/", Locale.getAvailableLocales()[i] + ".lang");
				if (tFile.exists()) {
					//LanguageHandler.sLangFile = new Configuration(tFile);
					Configuration langFile = new Configuration(tFile);
					LanguageHandler.sLangFileArray.add(langFile);
				}
			}
		} else {
		...
		EntityRegistry.registerModEntity(PrefixBlockFallingEntity.class, "gt6.MetaBlockFallingEntity", 0, this, 160, 1, T);
		...
		IL.Circuit_Selector.set(new ItemIntegratedCircuit("gt6.integrated_circuit"));
		// Initialises the Empty Slot Marker Item.
		IL.Empty_Slot.set(new ItemEmptySlot("gt6.empty_slot"));
