...
import static gregapi6.compat.thaumcraft.ICompatTC.*;
...
public class Compat_Recipes_Thaumcraft extends CompatMods {
	...
	@Override public void onPostLoad(FMLPostInitializationEvent aInitEvent) {OUT.println("GT_Mod: Doing TC Recipes and Research.");
		...
		COMPAT_TC.addResearch(tKey = WOOD_TO_CHARCOAL
			...
			, null
			, LH.add("gt6.research.page.1."+tKey, "You have discovered a way of making "+tMat.mNameLocal+" magically instead of using Coke Ovens for this purpose.<BR><BR>To create "+tMat.mNameLocal+" from Logs you first need an air-free environment, some vacuus essentia is needed for that, then you need to incinerate the Log using ignis essentia and wait until all the Water inside the Logs is burned away.<BR><BR>This method however doesn't create "+MT.Creosote.mNameLocal+" as byproduct.")
		...
		COMPAT_TC.addResearch(tKey = FILL_WATER_BUCKET
			...
			, null
			, LH.add("gt6.research.page.1." + tKey, "You have discovered a way of filling a Bucket with aqua essentia in order to simply get Water.")
		...
		COMPAT_TC.addResearch(tKey = TRANSZINC
			...
			, null
			, LH.add("gt6.research.page.1." + tKey, "You have discovered a way to multiply "+tMat.mNameLocal+" by steeping "+tMat.mNameLocal+" Nuggets in Metallum harvested from other Metals.")
		...
		COMPAT_TC.addResearch(tKey = IRON_TO_STEEL
			...
			, null
			, LH.add("gt6.research.page.1." + tKey, "You have discovered a way of making "+MT.Fe.mNameLocal+" harder by just re-ordering its components.<BR><BR>This Method can be used to create a Material called "+tMat.mNameLocal+", which is used in many non-Thaumaturgic applications.")
		...
		COMPAT_TC.addResearch(tKey = TRANSBRONZE
			...
			, Arrays.asList(TC.stack(TC.METALLUM, 5), TC.stack(TC.PERMUTATIO, 3), TC.stack(TC.INSTRUMENTUM, 3))
			, null
			, LH.add("gt6.research.page.1." + tKey, "You have discovered a way of creating Alloys using the already known transmutations of "+MT.Cu.mNameLocal+" and "+MT.Sn.mNameLocal+".<BR><BR>This Method can be used to create "+tMat.mNameLocal+" directly without having to go through an alloying process.")
			, COMPAT_TC.addCrucibleRecipe(tKey, OP.nugget.dat(tMat), OP.nugget.mat(tMat, 3), TC.stack(TC.METALLUM, 2), TC.stack(TC.INSTRUMENTUM, 1))
		);
		tMat = MT.Electrum;
		COMPAT_TC.addResearch(tKey = TRANSELECTRUM
			...
			, null
			, LH.add("gt6.research.page.1." + tKey, "Your discovery of "+MT.Bronze.mNameLocal+" Transmutation has lead you to the conclusion it works with other Alloys such as "+tMat.mNameLocal+" as well.")
		...
		COMPAT_TC.addResearch(tKey = ADVANCEDMETALLURGY
			...
			, null
			, LH.add("gt6.research.page.1." + tKey, "Now that you have discovered all the basic Metals, you can finally move on to the next Level of Magic Metallurgy and create more advanced Metals")
		);
		tMat = MT.Al;
		...
		COMPAT_TC.addResearch(tKey = CRYSTALLISATION
			...
			, null
			, LH.add("gt6.research.page.1." + tKey, "Sometimes when processing your Crystal Shards, they become a pile of Dust instead of the mostly wanted Shard.<BR><BR>You have finally found a way to reverse this Process by using Vitreus Essentia for recrystallising the Shards.")
		...
		COMPAT_TC.addResearch(tKey
			...
			, N
			, LH.add("gt6.research.page.1." + tKey, "")
		});*/
	}
}
