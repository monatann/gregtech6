...
import gregapi6.cover.covers.AbstractCoverAttachmentLogistics;
...
import gregapi6.tileentity.ITileEntityServerTickPre;
...
import gregapi6.tileentity.logistics.ITileEntityLogisticsStorage;
...
public class MultiTileEntityLogisticsCore extends TileEntityBase10MultiBlockBase implements ITileEntityEnergy, ITileEntityLogistics, ITileEntityServerTickPre, ITileEntityEnergyDataCapacitor, IMultiBlockEnergy, IMultiBlockFluidHandler, IFluidHandler {
	...
	public void readFromNBT2(NBTTagCompound aNBT) {
		...
		mCPU_Logic = aNBT.getInteger("gt6.cpu.logic");
		mCPU_Control = aNBT.getInteger("gt6.cpu.control");
		mCPU_Storage = aNBT.getInteger("gt6.cpu.storage");
		mCPU_Conversion = aNBT.getInteger("gt6.cpu.conversion");
		oCPU_Logic = aNBT.getInteger("gt6.cpu.logic.used");
		oCPU_Control = aNBT.getInteger("gt6.cpu.control.used");
		oCPU_Storage = aNBT.getInteger("gt6.cpu.storage.used");
		oCPU_Conversion = aNBT.getInteger("gt6.cpu.conversion.used");
	...
	public void writeToNBT2(NBTTagCompound aNBT) {
		...
		UT.NBT.setNumber(aNBT, "gt6.cpu.logic", mCPU_Logic);
		UT.NBT.setNumber(aNBT, "gt6.cpu.control", mCPU_Control);
		UT.NBT.setNumber(aNBT, "gt6.cpu.storage", mCPU_Storage);
		UT.NBT.setNumber(aNBT, "gt6.cpu.conversion", mCPU_Conversion);
		UT.NBT.setNumber(aNBT, "gt6.cpu.logic.used", oCPU_Logic);
		UT.NBT.setNumber(aNBT, "gt6.cpu.control.used", oCPU_Control);
		UT.NBT.setNumber(aNBT, "gt6.cpu.storage.used", oCPU_Storage);
		UT.NBT.setNumber(aNBT, "gt6.cpu.conversion.used", oCPU_Conversion);
	...
	public boolean checkStructure2() {
		...
		if (worldObj.blockExists(tX-2, tY, tZ-2) && worldObj.blockExists(tX+2, tY, tZ-2) && worldObj.blockExists(tX-2, tY, tZ+2) && worldObj.blockExists(tX+2, tY, tZ+2)) {
			...
			int[] blockId = {18200, 18201, 18202, 18203, 18204, 18012, 18299};
			
			for (int i = -2; i <= 2; i++) for (int j = -2; j <= 2; j++) for (int k = -2; k <= 2; k++) {
				if (i*i + j*j + k*k < 4) {
					if (ITileEntityMultiBlockController.Util.checkAndSetTarget(this, tX+i, tY+j, tZ+k, blockId[0], getMultiTileEntityRegistryID(), 0, MultiTileEntityMultiBlockPart.NOTHING)) {
						mCPU_Logic++;
						mCPU_Control++;
						mCPU_Storage++;
						mCPU_Conversion++;
					} else if (ITileEntityMultiBlockController.Util.checkAndSetTarget(this, tX+i, tY+j, tZ+k, blockId[1], getMultiTileEntityRegistryID(), 0, MultiTileEntityMultiBlockPart.NOTHING)) {
						mCPU_Logic += 4;
					} else if (ITileEntityMultiBlockController.Util.checkAndSetTarget(this, tX+i, tY+j, tZ+k, blockId[2], getMultiTileEntityRegistryID(), 0, MultiTileEntityMultiBlockPart.NOTHING)) {
						mCPU_Control += 4;
					} else if (ITileEntityMultiBlockController.Util.checkAndSetTarget(this, tX+i, tY+j, tZ+k, blockId[3], getMultiTileEntityRegistryID(), 0, MultiTileEntityMultiBlockPart.NOTHING)) {
						mCPU_Storage += 4;
					} else if (ITileEntityMultiBlockController.Util.checkAndSetTarget(this, tX+i, tY+j, tZ+k, blockId[4], getMultiTileEntityRegistryID(), 0, MultiTileEntityMultiBlockPart.NOTHING)) {
						mCPU_Conversion += 4;
					} else if (ITileEntityMultiBlockController.Util.checkAndSetTarget(this, tX+i, tY+j, tZ+k, blockId[5], getMultiTileEntityRegistryID(), 0, MultiTileEntityMultiBlockPart.NOTHING)) {
						// Well someone's a cheapstake. ;P
					} else {
						tSuccess = F;
					}
				} else if (i*i + j*j + k*k > 6) {
					if (!ITileEntityMultiBlockController.Util.checkAndSetTarget(this, tX+i, tY+j, tZ+k, blockId[5], getMultiTileEntityRegistryID(), 0, MultiTileEntityMultiBlockPart.ONLY_LOGISTICS & MultiTileEntityMultiBlockPart.ONLY_ENERGY_IN)) tSuccess = F;
				} else {
					if (!ITileEntityMultiBlockController.Util.checkAndSetTarget(this, tX+i, tY+j, tZ+k, blockId[6], getMultiTileEntityRegistryID(), 0, MultiTileEntityMultiBlockPart.ONLY_LOGISTICS)) tSuccess = F;
				}
			}
	...
	static {
		LH.add("gt6.tooltip.multiblock.logisticscore.1" , "5x5x5 Frame made out of 44 Galvanized Steel Walls (Power Input Here)");
		LH.add("gt6.tooltip.multiblock.logisticscore.2" , "3x3x3 Core of 27 of any Multiblock Processor Units (Customizable)");
		LH.add("gt6.tooltip.multiblock.logisticscore.3" , "The Six 3x3 Walls need to be 53 Ventilation Units");
		LH.add("gt6.tooltip.multiblock.logisticscore.4" , "Main centered at any Side facing outwards");
		LH.add("gt6.tooltip.multiblock.logisticscore.5" , "At least one of each Processor Type needed (Or use a Versatile one)");
		LH.add("gt6.tooltip.multiblock.logisticscore.6" , "You can replace CPUs with Walls should you not be able to afford that many.");
		LH.add("gt6.tooltip.multiblock.logisticscore.7" , "Logic Processors increase Operation Count by 1");
		LH.add("gt6.tooltip.multiblock.logisticscore.8" , "Control Processors increase Maximum Network Range by 1m (Cubic AoE)");
		LH.add("gt6.tooltip.multiblock.logisticscore.9" , "Storage Processors increase Buffer Size by 1 Stack or 16000L");
		LH.add("gt6.tooltip.multiblock.logisticscore.10", "Conversion Processors increase Throughput by 1 Stack or 16000L");
	}
	
	@Override
	public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
		aList.add(Chat.CYAN     + LH.get(LH.STRUCTURE) + ":");
		aList.add(Chat.WHITE    + LH.get("gt6.tooltip.multiblock.logisticscore.1"));
		aList.add(Chat.WHITE    + LH.get("gt6.tooltip.multiblock.logisticscore.2"));
		aList.add(Chat.WHITE    + LH.get("gt6.tooltip.multiblock.logisticscore.3"));
		aList.add(Chat.WHITE    + LH.get("gt6.tooltip.multiblock.logisticscore.4"));
		aList.add(Chat.YELLOW   + LH.get("gt6.tooltip.multiblock.logisticscore.5"));
		aList.add(Chat.YELLOW   + LH.get("gt6.tooltip.multiblock.logisticscore.6"));
		aList.add(Chat.YELLOW   + LH.get("gt6.tooltip.multiblock.logisticscore.7"));
		aList.add(Chat.YELLOW   + LH.get("gt6.tooltip.multiblock.logisticscore.8"));
		aList.add(Chat.YELLOW   + LH.get("gt6.tooltip.multiblock.logisticscore.9"));
		aList.add(Chat.YELLOW   + LH.get("gt6.tooltip.multiblock.logisticscore.10"));
							...
							for (byte tSide : ALL_SIDES_VALID) if (tCovers.mBehaviours[tSide] instanceof AbstractCoverAttachmentLogistics) {
								...
								if (tAdjacent.mTileEntity instanceof ITileEntityLogistics && ((ITileEntityLogistics)tAdjacent.mTileEntity).canLogistics(SIDE_ANY)) {
									...
									if (tCovers.mBehaviours[tSide] == CoverLogisticsFluidExport.INSTANCE) {
										FluidStack tFluid = FL.load(tCovers.mNBTs[tSide], "gt6.filter.fluid");
									...
									if (tCovers.mBehaviours[tSide] == CoverLogisticsItemExport.INSTANCE) {
										ItemStack tStack = ST.load(tCovers.mNBTs[tSide], "gt6.filter.item");
										if (ST.valid(tStack)) {
	...
	@Override public String getTileEntityName() {return "gt6.multitileentity.multiblock.logisticscore";}
}
