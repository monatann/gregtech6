...
public class MultiTileEntityMiniPortalDeepDark extends MultiTileEntityMiniPortal {
	...
	static {
		LH.add("gt6.tileentity.portal.deepdark.tooltip.1", "Only works between the Deep Dark and the Overworld!");
		LH.add("gt6.tileentity.portal.deepdark.tooltip.2", "Margin of Error to still work: 128 Meters.");
		LH.add("gt6.tileentity.portal.deepdark.tooltip.3", "Requires a Bedrockium Ingot for activation");
	}
	
	@Override
	public void addToolTips2(List<String> aList, ItemStack aStack, boolean aF3_H) {
		aList.add(Chat.CYAN     + LH.get("gt6.tileentity.portal.deepdark.tooltip.1"));
		aList.add(Chat.CYAN     + LH.get("gt6.tileentity.portal.deepdark.tooltip.2"));
		aList.add(Chat.ORANGE   + LH.get("gt6.tileentity.portal.deepdark.tooltip.3"));
	}
	...
	public void addThisPortalToLists() {
		if (MD.ExU.mLoaded && worldObj != null && isServerSide()) {
			if (worldObj.provider.dimensionId == DIM_OVERWORLD) {
				...
				for (MultiTileEntityMiniPortalDeepDark tPortal : sListDeepDarkSide) tPortal.findTargetPortal();
				findTargetPortal();
			} else if (WD.dimDD(worldObj)) {
				if (!sListDeepDarkSide.contains(this)) sListDeepDarkSide.add(this);
				for (MultiTileEntityMiniPortalDeepDark tPortal : sListWorldSide) tPortal.findTargetPortal();
				findTargetPortal();
			} else {
				setPortalInactive();
			}
	...
	@Override public String getTileEntityName() {return "gt6.multitileentity.portal.deepdark";}
}
