...
public class MultiTileEntityCoin extends TileEntityBase04MultiTileEntities implements IMTE_OnDespawn, IMTE_GetLifeSpan, IMTE_GetSubItems, IMTE_OnRegistration, IMTE_OnRegistrationClient, IMTE_GetExplosionResistance, IMTE_GetBlockHardness, IMTE_SetBlockBoundsBasedOnState, IMTE_AddCollisionBoxesToList, IMTE_GetCollisionBoundingBoxFromPool, IMTE_GetSelectedBoundingBoxFromPool, IMTE_GetLightOpacity, IMTE_AddToolTips, IMTE_OnPlaced, IMTE_SyncDataByteArray {
	...
	public void readFromNBT2(NBTTagCompound aNBT) {
		...
		for (int i = 0; i < mShape[0].length; i++) mShape[0][i] = UT.Code.getBitsS(aNBT.getShort("gt6.coin.shape.0."+i));
		for (int i = 0; i < mShape[1].length; i++) mShape[1][i] = UT.Code.getBitsS(aNBT.getShort("gt6.coin.shape.1."+i));
		for (int i = 0; i < mCoinStackSizes.length; i++) mCoinStackSizes[i] = aNBT.getByte("gt6.coin.stacksize."+i);
		mIsUnique = aNBT.getBoolean("gt6.coin.unique");
	...
	public void writeToNBT2(NBTTagCompound aNBT) {
		...
		UT.NBT.setBoolean(aNBT, "gt6.coin.unique", mIsUnique);
		aNBT.setString(NBT_MATERIAL, mMaterial.toString());
		for (int i = 0; i < mShape[0].length; i++) aNBT.setShort("gt6.coin.shape.0."+i, (short)UT.Code.getBits(mShape[0][i]));
		for (int i = 0; i < mShape[1].length; i++) aNBT.setShort("gt6.coin.shape.1."+i, (short)UT.Code.getBits(mShape[1][i]));
		for (int i = 0; i < mCoinStackSizes.length; i++) aNBT.setByte("gt6.coin.stacksize."+i, mCoinStackSizes[i]);
	}
	...
	static {
		LH.add("gt6.tooltip.coins.1", "Use the gregtech6.cfg inside your Minecraft Root Directory");
		LH.add("gt6.tooltip.coins.2", "to disable 3D Rendered Coins in World if they are too much");
		LH.add("gt6.tooltip.coins.3", "Render Lag for your System.");
	}
	
	@Override
	public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
		aList.add(LH.Chat.CYAN + mMaterial.getLocal() + (mIsUnique?" (Unique)":""));
		aList.add(LH.Chat.DGRAY + LH.get("gt6.tooltip.coins.1"));
		aList.add(LH.Chat.DGRAY + LH.get("gt6.tooltip.coins.2"));
		aList.add(LH.Chat.DGRAY + LH.get("gt6.tooltip.coins.3"));
	}
				...
				if (aEntity.worldObj.canPlaceEntityOnSide(MTE_REGISTRY.mBlock, UT.Code.roundDown(aEntity.posX)+OFFSETS_X[tSide], UT.Code.roundDown(aEntity.posY)+OFFSETS_Y[tSide], UT.Code.roundDown(aEntity.posZ)+OFFSETS_Z[tSide], F, SIDE_TOP, aEntity, aStack)) {
					...
					for (int i = 0; i < mCoinStackSizes.length; i++) {
						...
						if (tDifference > 0) {
							...
							tNBT.setByte("gt6.coin.stacksize."+i, (byte)tDifference);
						}
					}
	...
	@Override public String getTileEntityName() {return "gt6.multitileentity.coin";}
}
