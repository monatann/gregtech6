...
public class BlockBaleGrass extends BlockBaseBale {
	...
	static {
		LH.add("gt6.tooltip.bale", "Bale dries or rots when placed");
		LH.add("gt6.tooltip.bale.dry", "It would dry if placed here at the moment");
		LH.add("gt6.tooltip.bale.rot", "It would rot if placed here at the moment");
	}
	...
	public void addInformation(ItemStack aStack, int aMeta, EntityPlayer aPlayer, List<String> aList, boolean aF3_H) {
		...
		if ((aMeta & 3) == 0) {
			aList.add(LH.Chat.CYAN + LH.get("gt6.tooltip.bale"));
			if (aPlayer != null) {
				if (aPlayer.worldObj.provider.isHellWorld) {
					aList.add(LH.Chat.YELLOW + LH.get("gt6.tooltip.bale.dry"));
				} else {
					...
					if (tBiome.rainfall > 0.8F || (aPlayer.worldObj.isRaining() && tBiome.rainfall > 0 && aPlayer.worldObj.getPrecipitationHeight(aX, aZ) <= aY+2)) {
						aList.add(LH.Chat.ORANGE + LH.get("gt6.tooltip.bale.rot"));
					} else {
						aList.add(LH.Chat.YELLOW + LH.get("gt6.tooltip.bale.dry"));
					}
				}
