...
public class Behavior_Spray_Foam_Remover extends AbstractBehaviorDefault {
	...
	public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ, byte aSide, float hitX, float hitY, float hitZ) {
		...
		if (ST.equal(aStack, mUsed, T)) {
			...
			if (tRemoved > 0) {
				UT.Sounds.send(aWorld, SFX.IC_SPRAY, 1.0F, 1.5F, aX, aY, aZ);
	...
	static {
		LH.add("gt6.behaviour.foamremoverspray.tooltip", "Can remove C-Foam");
		LH.add("gt6.behaviour.removerspray.uses", "Remaining Uses:");
		LH.add("gt6.behaviour.unstackable", "Not usable when stacked!");
	}
	
	@Override
	public List<String> getAdditionalToolTips(MultiItem aItem, List<String> aList, ItemStack aStack) {
		aList.add(LH.get("gt6.behaviour.foamremoverspray.tooltip"));
		NBTTagCompound tNBT = aStack.getTagCompound();
		long tRemaining = (ST.equal(aStack, mFull, T)?mUses:tNBT==null?0:tNBT.getLong("gt6.remaining"));
		aList.add(LH.get("gt6.behaviour.removerspray.uses") + " " + (tRemaining / 10) + "." + (tRemaining % 10));
