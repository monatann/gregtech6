...
public class Behavior_Spray_Foam extends AbstractBehaviorDefault {
	...
	public Behavior_Spray_Foam(ItemStack aEmpty, ItemStack aUsed, ItemStack aFull, long aUses, byte aColor, boolean aOwned) {
		...
		LH.add("gt6.behaviour.foamspray."+mColor+".tooltip", "Can place " + DYE_NAMES[mColor] + " C-Foam");
	}
	...
	public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ, byte aSide, float hitX, float hitY, float hitZ) {
		...
		UT.NBT.setNumber(tNBT, "gt6.remaining", tUses);
	...
	static {
		LH.add("gt6.behaviour.foamspray.uses", "Remaining Uses:");
		LH.add("gt6.behaviour.unstackable", "Not usable when stacked!");
	}
	
	@Override
	public List<String> getAdditionalToolTips(MultiItem aItem, List<String> aList, ItemStack aStack) {
		aList.add(LH.get("gt6.behaviour.foamspray."+mColor+".tooltip"));
		NBTTagCompound tNBT = aStack.getTagCompound();
		long tRemaining = (ST.equal(aStack, mFull, T)?mUses:tNBT==null?0:tNBT.getLong("gt6.remaining"));
		aList.add(LH.get("gt6.behaviour.foamspray.uses") + " " + (tRemaining / 10) + "." + (tRemaining % 10));
